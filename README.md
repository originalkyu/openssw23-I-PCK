...
#### 요약 및 결론
[요약]  
(1) 시각적으로 비교할 때 가우시안 노이즈를 제거할 때는 NAFNet이 우수하고, Pepper and salt 노이즈를 제거할 때는 opencv의 Median Blur 함수가 우수하다.  
(2) psnr 점수를 기준으로 Pepper and salt노이즈를 제거할 때 MedianBlur함수가 NAFNet을 이용할 때보다 낫다.  
(3) (2) 의 경우를 제외한 나머지의 비교에서는 psnr점수와 ssim점수 기준으로, 어느 방식이 유의미하게 낫다고하기 어려웠다.  
(4) 이미지를 처리하는 속도는 opencv가 NAFNet보다 압도적으로 빨랐다.

[결론]  
[요약]의 (1),(2),(3)를 고려하면 psnr점수와 ssim점수는 시각적으로 확인했을 때의 차이를 잘 반영하지 못하는 지표이다.  
[요약]의 (1),(2),(4)을 고려하면 Pepper and salt 노이즈를 제거할 때는 MedianBlur함수가 NAFNet보다 빠르고 효과가 좋다.  
[요약]의 (1),(4)을 고려하면 가우시안 노이즈를 제거할 때 속도가 중요하다면 filter2D()함수를 이용하고, 시각적으로 나은 결과물이 필요하다면 NAFNet이 낫다.  
